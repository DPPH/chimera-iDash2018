...
def sigmoid(x, ARGUMENT_PRECISION):
    x = x / FXnum(2 ** (PRECISION_NUM - ARGUMENT_PRECISION), PRECISION)
    x = x * FXnum(2 ** (PRECISION_NUM - ARGUMENT_PRECISION), PRECISION)
    ans = None
    if x > 30:
        ans = FXnum(1, PRECISION)
    elif x < -30:
        ans = FXnum(0, PRECISION)
    ans = FXnum(1 / (1 + math.e ** (-x)), PRECISION)
    # ans = ans / FXnum(2 ** 10, PRECISION)
    # ans = ans * FXnum(2 ** 10, PRECISION)
    return ans
...
for SIGMOID_PRECISION in [9]:
    for STEP in [0.5, 1, 2, 4, 8]:
        for ITERATION_NUM in [10, 50]:
            ...
            for _ in range(ITERATION_NUM):
                ...
                temp_vec_2 = [FXnum(Y_fx[i] - sigmoid(temp_vec_1[i], SIGMOID_PRECISION), PRECISION) for i in range(n)]
            ...
            p_vec = [sigmoid(X_beta_vec[i], SIGMOID_PRECISION) for i in range(n)]
            ...
            plotter.value_iteration_plot(
                ...
                "/results/gradient_descent_orthogonal_fixed_precision_sigmoid/"
                + format("precision_%d/argument_precision_%d/beta_graph/step_%.2f" % (PRECISION_NUM, SIGMOID_PRECISION, STEP)),
            ...
            plotter.value_iteration_plot(
                ...
                + format("step=%.2f, precision=%d bits" % (STEP, PRECISION_NUM)),
                "/results/gradient_descent_orthogonal_fixed_precision_sigmoid/"
                + format("precision_%d/argument_precision_%d/error_graph/step_%.2f" % (PRECISION_NUM, SIGMOID_PRECISION, STEP)),
                format("/iter_%d" % iteration_cnt)
            )
