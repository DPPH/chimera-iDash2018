...
void mul64(BigTorusRef out, int64_t a, const BigTorusRef &in, UINT64 out_limb_prec) {
    ...
    assert_dramatically(out_limb_prec >= 1, "precision too low");
    assert_dramatically(out_limb_prec <= in.params->torus_limbs, "requested input precision too high");

    UINT64 *out_ptr = out.limbs + out.params->torus_limbs - out_limb_prec;
    UINT64 *in_ptr = in.limbs + in.params->torus_limbs - out_limb_prec;
    if (a > 0) {
        mpn_mul_1(out_ptr, in_ptr, out_limb_prec, a);
    } else {
        a = -a;
        mpn_mul_1(out_ptr, in_ptr, out_limb_prec, a);
        mpn_neg(out_ptr, out_ptr, out_limb_prec);
    }
}
